...
Read a local TTF, OTF, or WOFF2 font and report the Unicode scripts mapped by
its character map. Inspection detects the source format from file content and
does not load providers, use the result cache, or change the input file.
...
Plain output includes the file identity, font family when present, glyph
count, Unicode data version, and a table like this:
...
Inspection supports TTF, OTF, and WOFF2. It rejects font collections and the
legacy WOFF1, dfont, PFB, and PFM formats. These formats need different
character-map parsers, and PFM files contain metrics rather than glyphs.
