...
Local conversion is a separate pipeline:

```text
local TTF, OTF, or WOFF2
  -> content-based format detection
  -> WOFF2 encode or decode
  -> output validation
  -> atomic no-replace file commit
```

It does not enter provider discovery, aggregation, ranking, cache, download,
or TUI code.
